		...
		req.session.id_perfil && req.session.id_perfil==2){
			DBCONEXION.iniciar_conexion(function (conexion_db) {
				if (conexion_db!=false) {
					...
					BOT.all(conexion_db_ON,function(err_0,result_0){
...
router.post('/registrar_nuevo', function(req, res) {
	if(req.session.id_perfil && req.session.id_perfil==1 ){
		...
		DBCONEXION.iniciar_conexion(function (conexion_db) {
			if (conexion_db!=false) {
				conexion_db_ON = conexion_db;
				BOT.get_existencia(conexion_db_ON,req.body,function (err_v,result_v) {
			        if(result_v==false)
			        {				
			    		BOT.registrar_nuevo(conexion_db_ON,req.body,function (result) {
					        if(result==false)
					        {
					        	DBCONEXION.cerrar_conexion(conexion_db_ON);
					            req.session.error_bot = true,
								req.session.mensaje_error = " No se pudo registrar el bot : "+req.body.nombre;
						...
						DBCONEXION.cerrar_conexion(conexion_db_ON);
						req.session.error_bot = true,
						req.session.mensaje_error = "El bot : "+req.body.nombre+", ya existe.";
						res.redirect('/admin/bots');						
					}
				});					
			}
...
router.post('/editar', function(req, res) {
	if(req.session.id_perfil && req.session.id_perfil==1 ){
		...
		DBCONEXION.iniciar_conexion(function (conexion_db) {
			if (conexion_db!=false) {
				conexion_db_ON = conexion_db;
				BOT.editar(conexion_db_ON,req.body,function (result) {
			        if(result==false)
			        {
			        	DBCONEXION.cerrar_conexion(conexion_db_ON);
			            //res.json(err);
			            req.session.error_bot = true,
						req.session.mensaje_error = "No se pudo editar al bot : "+req.body.nombre;
						res.redirect('/admin/bots');
			        }else{
						DBCONEXION.cerrar_conexion(conexion_db_ON);
						res.redirect('/admin/bots');
...
router.post('/cambiar_estado', function(req, res) {
	if(req.session.id_perfil && req.session.id_perfil==1 ){
		...
		DBCONEXION.iniciar_conexion(function (conexion_db) {
			if (conexion_db!=false) {
				conexion_db_ON = conexion_db;
	    		BOT.cambiar_estado(conexion_db_ON,req.body,function (result) {
			        if(result==false)
			        {
			        	DBCONEXION.cerrar_conexion(conexion_db_ON);
			            req.session.error_bot = true,
