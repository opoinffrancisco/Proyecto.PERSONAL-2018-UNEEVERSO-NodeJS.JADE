		...
		req.session.id_perfil && req.session.id_perfil==2){
			DBCONEXION.iniciar_conexion(function (conexion_db) {
				if (conexion_db!=false) {
					...
					GESTION_URL.get_a_url(conexion_db_ON,function(err_0,result_0){
						//console.log(err_0,result_0);
						if(err_0)
						{
							res.json(err_0);
						}else{			
							res.render('admin/automatizados.jade', {
								dias_auto : 7,
	...
	if(req.session.id_perfil && req.session.id_perfil==1 || 
		req.session.id_perfil && req.session.id_perfil==2){
			DBCONEXION.iniciar_conexion(function (conexion_db) {
				if (conexion_db!=false) {
					conexion_db_ON = conexion_db;
					GESTION_URL.get_a_url_buscar(conexion_db_ON, req.body,function(err_0,result_0){
						//console.log(err_0,result_0);
						if(err_0)
						{
							res.json(err_0);
...
router.post('/reenvio', function(req, res) {
	if(req.session.id_perfil && req.session.id_perfil==1){
			DBCONEXION.iniciar_conexion(function (conexion_db) {
				if (conexion_db!=false) {
					conexion_db_ON = conexion_db;
		    		GESTION_URL.reenviar_urls(conexion_db_ON,req.body,function (err) {
				        if(err)
				        {
							...
							DBCONEXION.cerrar_conexion(conexion_db_ON);
							res.redirect('/admin/automatizados');
						};
					});
				};
			});
